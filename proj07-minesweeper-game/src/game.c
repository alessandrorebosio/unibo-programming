...
enum status game_update(struct game *G, enum input input)
{
	...
	case FLAG:
		...
		break;
	case EXPAND:
		c = expand_board(G->board, G->rows, G->cols, G->cursor_row, G->cursor_col);
		// gameover if either an unflagged  mine or all the free cells have been  displayed
		if (c == -1 || (G->displayed += c) == G->rows * G->cols - G->mines)
			stat = GAMEOVER;
		break;
	case DISPLAY:
		c = display_board(G->board, G->rows, G->cols, G->cursor_row, G->cursor_col);
