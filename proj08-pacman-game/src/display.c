...
static int select_border(char **A, unsigned int nrow, unsigned int ncol, unsigned int i, unsigned int j)
{
	if (A[i][j] == ULCORN_SYM)
	{
		if (i < nrow - 1 && (A[i + 1][j] == VWALL_SYM || A[i + 1][j] == URCORN_SYM) && j < ncol - 1 && (A[i][j + 1] == HWALL_SYM || A[i][j + 1] == URCORN_SYM))
			return ACS_ULCORNER;

		if (i > 0 && (A[i - 1][j] == VWALL_SYM || A[i - 1][j] == URCORN_SYM) && j > 0 && (A[i][j - 1] == HWALL_SYM || A[i][j - 1] == URCORN_SYM))
			return ACS_LRCORNER;
	}
	else if (A[i][j] == URCORN_SYM)
	{
		if (i < nrow - 1 && (A[i + 1][j] == VWALL_SYM || A[i + 1][j] == ULCORN_SYM) && j > 0 && (A[i][j - 1] == HWALL_SYM || A[i][j - 1] == ULCORN_SYM))
			return ACS_URCORNER;

		if (i > 0 && (A[i - 1][j] == VWALL_SYM || A[i - 1][j] == ULCORN_SYM) && j < ncol - 1 && (A[i][j + 1] == HWALL_SYM || A[i][j + 1] == ULCORN_SYM))
			return ACS_LLCORNER;
	}
	return A[i][j];
}
...
static void display_board(char **A, unsigned int nrow, unsigned int ncol)
{
	...
	for (i = 0; i < nrow; i++)
		for (j = 0; j < ncol; j++)
		{
			...
			{
			case VWALL_SYM:
				attron(COLOR_PAIR(DARK_WALL_PAIR));
...
{
	int i = pos.i, j = pos.j;
	int col = stat == SCARED_NORMAL ? FRIGHT_PAIR : stat == EYES		  ? EYES_PAIR
												: stat == SCARED_BLINKING ? FRIGHT_FLASH_PAIR
																		  : CH_PAIR[(id % 4) + 1];

	attron(COLOR_PAIR(col));
#ifdef BLINK
	if (stat == SCARED_BLINKING)
		attron(A_BLINK);
#endif
...
void display_arena(void)
{
	...
	if (M != NULL)
	{
		...
		{
			pos = arena_ghost_position(g);
			pos.j *= 3;
			display_ghost(g, pos, arena_ghost_status(g));
		}

		// Print pacman
		pos = arena_pacman_position();
		pos.j *= 3;
